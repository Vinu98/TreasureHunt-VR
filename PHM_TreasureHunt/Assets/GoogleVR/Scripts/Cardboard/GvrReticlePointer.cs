  ...
  public override void OnPointerHover(RaycastResult raycastResultResult, bool isInteractive) {
    SetPointerTarget(raycastResultResult.worldPosition, isInteractive);
		...
		if (gazedAt != null && gazeStartTime > 0.0f) 
			if (Time.time - gazeStartTime > delayTime && ExecuteEvents.CanHandleEvent<TimedGaze> (gazedAt)) {
				gazeStartTime = -1.0f;
				ExecuteEvents.Execute (gazedAt, null, (TimedGaze handler, BaseEventData data) => handler.handleTimedInput());
			}
  }
